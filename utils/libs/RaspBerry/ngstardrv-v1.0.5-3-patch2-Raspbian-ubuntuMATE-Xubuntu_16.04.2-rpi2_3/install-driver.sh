...
echo "NITGEN USB Finkey HAMSTER II/III Driver Install "
...
# use dynamic allocate usb deviceid ?
is_dyn_usb_devfile_use=""
if [ -n "`mount | grep sysfs`" -a -d "/etc/udev" ] ; then
...
if [ -z "${is_dyn_usb_devfile_use}" ] ; then
    ...
    #create 8 nodes for the device(All User permission(R/W) allowed..)
    chmod 0666 /dev/${device}0
    chmod 0666 /dev/${device}1
    chmod 0666 /dev/${device}2
    chmod 0666 /dev/${device}3
    chmod 0666 /dev/${device}4
    chmod 0666 /dev/${device}5
    chmod 0666 /dev/${device}6
    chmod 0666 /dev/${device}7
else
...
echo "NITGEN USB Finkey HAMSTER II/III Driver sucessfully installed "
